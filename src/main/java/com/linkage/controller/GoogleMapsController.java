...
public class GoogleMapsController extends BaseController {
    ...
    public GoogleMapsController(LinkageConfiguration configuration, Validator validator) {
        super(configuration, validator);googleMapsService = new GoogleMapsService(configuration);
    ...
    public Response nearbySearch(@Context HttpServletRequest request,
            NearbySearchSchema body) {
        ...
        receiver = (Map<String,Object>)this.googleMapsService.nearbySearch(body);
        return Response.status(Response.Status.OK)
